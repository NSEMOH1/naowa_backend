...
exports.generateBalanceSheetReport = exports.generateProfitLossReport = exports.generateTrialBalanceReport = exports.getAllMembersFinancialSummary = exports.generateMemberFinancialSummary = exports.generateLoanRepaymentReport = exports.generateFinancialReport = void 0;
const database_1 = require("../config/database");
const node_cache_1 = __importDefault(require("node-cache"));
const savingService_1 = require("./savingService");
const reportCache = new node_cache_1.default({ stdTTL: 3600 });
const getCacheKey = (reportName, params) => {
    return `${reportName}:${JSON.stringify(params)}`;
};
const generateFinancialReport = async (startDate, endDate, includeMemberDetails = false) => {
    ...
    const income = transactions
        .filter((t) => ["LOAN_REPAYMENT", "SAVINGS_DEPOSIT", "FEE"].includes(t.type))
...
const generateLoanRepaymentReport = async (startDate, endDate, includeMemberDetails = false) => {
...
exports.generateLoanRepaymentReport = generateLoanRepaymentReport;
const generateMemberFinancialSummary = async (memberId, startDate, endDate) => {
    ...
    const [member, transactions, loans, savings] = await Promise.all([
        database_1.prisma.member.findUnique({
            where: { id: memberId },
        }),
        ...
        database_1.prisma.saving.findMany({
            where: { memberId },
            include: {
                category: true,
            },
    ...
    const loanSummary = {
        ...
        totalRepaid: loans.reduce((sum, l) => sum +
            l.repayments.reduce((repSum, r) => r.status === "PAID" ? repSum + Number(r.amount) : repSum, 0), 0),
    ...
    const totalSavings = await (0, savingService_1.getMemberTotalSavings)(member.id);
    const report = {
        member: {
            id: member.id,
...
exports.generateMemberFinancialSummary = generateMemberFinancialSummary;
const getAllMembersFinancialSummary = async (startDate, endDate) => {
    ...
    const memberData = members.map((member) => {
        ...
        const totalSavings = (0, savingService_1.getMemberTotalSavings)(member.id);
        return {
            id: member.id,
            name: `${member.first_name} ${member.last_name}`,
            serviceNumber: member.service_number,
            ...
            totalTransactionAmount: memberTransactions.reduce((sum, t) => sum + Number(t.amount), 0),
            loanCount: memberLoans.length,
            activeLoanCount: memberLoans.filter((l) => l.status === "ACTIVE").length,
            totalLoanAmount: memberLoans.reduce((sum, l) => sum + Number(l.approvedAmount), 0),
            savingsCount: memberSavings.length,
            totalSavingsAmount: memberSavings.reduce((sum, s) => sum + Number(s.amount), 0),
        };
    });
    const report = {
        period: startDate && endDate ? { start: startDate, end: endDate } : null,
        ...
        summary: {
            totalTransactions: transactions.length,
            totalTransactionAmount: transactions.reduce((sum, t) => sum + Number(t.amount), 0),
            totalLoans: loans.length,
            totalLoanAmount: loans.reduce((sum, l) => sum + Number(l.approvedAmount), 0),
            totalSavings: savings.length,
...
exports.getAllMembersFinancialSummary = getAllMembersFinancialSummary;
const generateTrialBalanceReport = async (asOfDate) => {
    ...
    const totalLoans = loans.reduce((sum, loan) => sum + Number(loan.approvedAmount || loan.amount), 0);
    const totalSavings = savings.reduce((sum, saving) => sum + Number(saving.amount), 0);
    const totalRepayments = repayments.reduce((sum, repayment) => sum + Number(repayment.amount), 0);
    return {
        asOfDate,
        accounts: [
...
exports.generateTrialBalanceReport = generateTrialBalanceReport;
const generateProfitLossReport = async (startDate, endDate) => {
    const transactions = await database_1.prisma.transaction.findMany({
        where: {
            createdAt: {
                gte: startDate,
...
exports.generateProfitLossReport = generateProfitLossReport;
const generateBalanceSheetReport = async (asOfDate) => {
...
const getTotalLoanReceivables = async (asOfDate) => {
...
const getTotalSavingsLiabilities = async (asOfDate) => {
    const savings = await database_1.prisma.saving.findMany({
        where: {
            status: "COMPLETED",
            createdAt: {
