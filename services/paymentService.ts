...
export const processPayment = async (data: {
  ...
  if (paymentType === "SAVINGS_DEPOSIT") {
    ...
    return await processLoanRepayment(memberId, amount, reference, loanId);
  }

  throw new Error("Invalid payment type");
};

const processSavingsDeposit = async (
  memberId: string,
  amount: number,
  reference: string
) => {
    ...
    if (allPaid && balanceIsZero) {
      ...
      await tx.notification.create({
        data: {
          ...
          message: `Congratulations! You have fully paid your loan of ₦${Number(
            loan.approvedAmount
          ).toLocaleString()}. Total repaid: ₦${totalPaid.toLocaleString()}. You're now eligible to apply for a new loan!`,
          type: "SUCCESS",
      ...
      await tx.notification.create({
        data: {
          ...
          message: `Your loan repayment of ₦${amount.toLocaleString()} has been processed successfully. ${
            balanceIsZero
